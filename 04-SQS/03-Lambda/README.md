...
O `sls deploy` empacota o código, gera um template do CloudFormation a partir do `serverless.yml` e sobe (ou atualiza) uma *stack* na conta AWS, criando a função Lambda, o *role* de execução e o Event Source Mapping com a `demoqueue`. O comando é **idempotente**: rodar `sls deploy` de novo sobre uma stack já publicada apenas aplica o diff, sem duplicar recursos.
